...
parser.add_argument(
    ...
    "--output_dir",
    type=str,
    default="./dataset/stackoverflow-paired",
    help="saved dataset output dir",
)
parser.add_argument(
    "-p", "--num_proc", type=int, default=32, help="numbers of processors to use"
)
parser.add_argument(
    "-s", "--partition_size", default=100000, help="size of each set ", type=int
)
...
try:
    train_set_filtered = train_set.filter(
        lambda s: "Stackoverflow" in s["metadata"][0], num_proc=CPUS
    )
    test_set_filtered = test_set.filter(
        lambda s: "Stackoverflow" in s["metadata"][0], num_proc=CPUS
    )
...
# saving to disk
train_set_filtered.select([i for i in range(300000)]).save_to_disk(
    f"{OUTPUT_DIR}/train", num_proc=CPUS
)
